"use client";
import { Pagination, PaginationContent, PaginationEllipsis, PaginationLink } from "@/components/ui/pagination";
    ...
    for (let i = 1; i <= pageCount; i++) {
      if (i === 1 || i === pageCount || (i >= currentPage - 1 && i <= currentPage + 1)) {
        paginationLinks.push(
          <PaginationLink key={i} onClick={() => changePage(i)} isActive={currentPage === i}>
            {i}
          </PaginationLink>
        );
      }
  ...
  return (
    <Pagination>
      <PaginationContent className=" *:cursor-pointer">
        ...
        <div className="max-sm:hidden flex flex-row items-center gap-1">{generatePaginationLinks()}</div>
        <Button
          variant="ghost"
